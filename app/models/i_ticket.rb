...
class ITicket < ActiveRecord::Base
  ...
  def self.tickets_list(issue_id, current_user_id = nil)
    ...
    r_uids.each do |hash|
      ...
      if main_ticket.iaccesses.empty?
        ...
        if main_ticket.iaccesses.first.confirmed_by_id?
          ticket[:status] = l(:at_confirmed_at) + main_ticket.iaccesses.first.confirmed_at.in_time_zone(tz).to_s(:atf)
          ticket[:status_id] = 4
          ticket[:user_id] = main_ticket.iaccesses.first.confirmed_by_id
          ticket[:user_name] = User.where(:id => main_ticket.iaccesses.first.confirmed_by_id).first.name
        else
          if main_ticket.iaccesses.first.granted_by_id?
            ticket[:status] = l(:at_granted_at) + main_ticket.iaccesses.first.granted_at.in_time_zone(tz).to_s(:atf)
            ticket[:status_id] = 3
            ticket[:user_id] = main_ticket.iaccesses.first.granted_by_id
            ticket[:user_name] = User.where(:id => main_ticket.iaccesses.first.granted_by_id).first.name
            if !current_user_id.nil?
              if ticket[:i_resource_id].in?(granted_resources)
    ...
    if ITicket.active.where(:issue_id => issue_id).count > 0 
      ...
      ITicket.active.where(:issue_id => issue_id).each do |iticket|
        if iticket.iaccesses.count > 0
          if iticket.iaccesses.first.granted_by_id?
            i += 1
          end
        end
      end
  ...
  def self.check_itickets_for_confirmed(issue_id)
    if ITicket.active.where(:issue_id => issue_id).count > 0 
      ...
      ITicket.active.where(:issue_id => issue_id).each do |iticket|
        if iticket.iaccesses.count > 0
          if iticket.iaccesses.first.confirmed_by_id?
            i += 1
          end
        end
      end
